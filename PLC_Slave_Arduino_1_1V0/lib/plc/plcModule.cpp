...
void PlcModule ::begin(uint32_t plcBaudRate)
{
  ...
  Serial.write(msg1); // reboot PLC
  delay(200); // CRITICAL. Must wait for the device to reboot so it can take next command 
...
int PlcModule ::sendPacket(char *payload)
{
  ...
  Serial.write(_txBuffer); // Send ASCII character to the PLC (UART port)
  Serial.flush();          // wait for transmission of data to complete. Slows the program down significantly
...
int PlcModule ::recvPacket()
{
  ...
  if (Serial.available())
  {
    ...
    *(rxPacket.payload + rxPacket.dLen) = '\0';
    ...
    txPacket.srcAddr = rxPacket.destAddr;
    txPacket.destAddr = rxPacket.srcAddr;
    txPacket.port = rxPacket.port;
...
int PlcModule ::processRxPacket(uint8_t devAddr)
{
  if (rxPacket.destAddr == devAddr)
  {
    ...
    {
      // Serial.write("SLAVE ACK\r"); // if YES, acknowledge (send an ACK)
    }
    else
    {
      // Serial.write("ERROR: SLAVE NACK\r"); // if NOT, No acknowledge (send a NACK)
      led_blink(3, 100, 50);
      return (-1);
    }
    led_blink(2, 20, 20);
