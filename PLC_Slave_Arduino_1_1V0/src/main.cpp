...
//
// Packet format
// [PacketLength(2), DeviceAddr(2), Port(2), Command(2), Payload(N), Checksum(2)]
// DeviceAddr = 1
// Port(s):
//     1 = LCD Display
//     2 = Temp Sensor
...
uint8_t tempSensor = 1; // the temp sensor number (if more than 1)

// Globals
char version[] = "Ver SL1-191122-1428";
...
void loop()
{
  ...
  temperature = getTemperature(tempSensor); // get a temperature reading
  dtostrf(temperature, 5, 2, fstr);         // convert float to a string
  strcpy(Plc.txPacket.payload, fstr);
  sprintf(str, "S1 temp: %s C", fstr);
  Disp.println(str, DISP_LINE_1);

  if (Plc.available(PLC_SLAVE_1))
  {
    ...
    if (Plc.rxPacket.destAddr == PLC_SLAVE_1) // each PLC_SLAVE has its own capabilities
    {
      if (Plc.rxPacket.port == PLC_PORT_0)
      {
        switch (Plc.rxPacket.command)
        {
        case 1: // [1 0 1]: Get version
      ...
      else if (Plc.rxPacket.port == PLC_PORT_1)
      {
        switch (Plc.rxPacket.command)
        {
        case 1: // [1 1 1]: Display sent message (rxBuffer payload) on the LCD
          Plc.sendPacket(Plc.txPacket.payload);
          // Plc.sendPacket(fstr);
          Disp.println(Plc.rxPacket.payload, DISP_LINE_2);
          printPacket(DISP_LINE_3, 't'); // Display tx packet elements on the LCD
      ...
      /**********  [SlAddr 2 x] **********/
      else if (Plc.rxPacket.port == PLC_PORT_2)
      {
        switch (Plc.rxPacket.command)
        {
        case 1: // [1 2 1]
